...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define TIMER_FREQ_HZ   90000000UL   // 90 MHz (APB1)
...
volatile uint32_t periodo_cuentas = 0;
volatile uint8_t primera_captura = 0;
volatile uint8_t nueva_medicion = 0;

uint32_t buffer_filtro[FILTER_SIZE];
uint8_t indice_filtro = 0;
uint8_t contador_muestras = 0;
uint32_t suma_filtro = 0;
uint32_t frecuencia_filtrada = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_TIM4_Init();
  /* USER CODE BEGIN 2 */
  // Iniciar TIM2 (Input Capture)
  HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_1);

  // Iniciar PWM (señales cuadradas)
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM2 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
    uint32_t captura_actual = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);
    if (primera_captura == 0) {
      captura1 = captura_actual;
      primera_captura = 1;
    } else {
      captura2 = captura_actual;
      nueva_medicion = 1;
      primera_captura = 0;
    }
  }
